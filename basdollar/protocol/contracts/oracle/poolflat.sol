...
library Constants {
    ...
    uint256 private constant ORACLE_RESERVE_MINIMUM = 1000e19; // 10,000 BUSD

    /* Bonding */
    uint256 private constant INITIAL_STAKE_MULTIPLE = 1e6; // 100 BASD -> 100M BASDS
    ...
    /* DAO */
    uint256 private constant ADVANCE_INCENTIVE = 1e20; // 100 BASD
...
/*
    ...
    limitations under the License.
*/
contract Permittable is BEP20Detailed, BEP20 {
...
/*
    ...
    limitations under the License.
*/
contract Dollar is IDollar, MinterRole, BEP20Detailed, Permittable, BEP20Burnable {

    constructor()
    BEP20Detailed("Binance Agile Set Dollar", "BASD", 18)
    Permittable()
    public
    { }
...
contract Oracle is IOracle {
    ...
    IPancakeswapV2Pair internal _pair;
    ...
    function setup() public onlyDao {
        _pair = IPancakeswapV2Pair(IPancakeswapV2Factory(PANCAKESWAP_FACTORY).createPair(_dollar, busd()));
    ...
    function updatePrice() private returns (Decimal.D256 memory) {
        ...
        PancakeswapV2OracleLibrary.currentCumulativePrices(address(_pair));
...
contract PoolGetters is PoolState {
    ...
    function Pancakev2() public view returns (IBEP20) {
        return _state.provider.Pancakev2;
    }
...
contract Liquidity is PoolGetters {
    ...
    function addLiquidity(uint256 dollarAmount) internal returns (uint256, uint256) {
        ...
        uint256 busdAmount = (reserveA == 0 && reserveB == 0) ?
             dollarAmount :
             PancakeswapV2Library.quote(dollarAmount, reserveA, reserveB);

        address pair = address(Pancakev2());
        IBEP20(dollar).transfer(pair, dollarAmount);
        IBEP20(busd).transferFrom(msg.sender, pair, busdAmount);
        return (busdAmount, IPancakeswapV2Pair(pair).mint(address(this)));
    }

    // overridable for testing
    function getReserves(address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        (address token0,) = PancakeswapV2Library.sortTokens(tokenA, tokenB);
        (uint reserve0, uint reserve1,) = IPancakeswapV2Pair(PancakeswapV2Library.pairFor(PANCAKESWAP_FACTORY, tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }
}
...
contract Pool is PoolSetters, Liquidity {
    ...
    constructor(address dollar, address Pancakev2) public {
        _state.provider.dao = IDAO(msg.sender);
        _state.provider.dollar = IDollar(dollar);
        _state.provider.Pancakev2 = IBEP20(Pancakev2);
    }
    ...
    event Provide(address indexed account, uint256 value, uint256 lessBusd, uint256 newPancakev2);

    function deposit(uint256 value) external onlyFrozen(msg.sender) notPaused {
        Pancakev2().transferFrom(msg.sender, address(this), value);
    ...
    function withdraw(uint256 value) external onlyFrozen(msg.sender) {
        Pancakev2().transfer(msg.sender, value);
    ...
    function provide(uint256 value) external onlyFrozen(msg.sender) notPaused {
        ...
        (uint256 lessBusd, uint256 newPancakev2) = addLiquidity(value);

        uint256 totalRewardedWithPhantom = totalRewarded().add(totalPhantom()).add(value);
        uint256 newPhantomFromBonded = totalRewardedWithPhantom.mul(newPancakev2).div(totalBonded());

        incrementBalanceOfBonded(msg.sender, newPancakev2);
        incrementBalanceOfPhantom(msg.sender, value.add(newPhantomFromBonded));


        balanceCheck();

        emit Provide(msg.sender, value, lessBusd, newPancakev2);
    }

    function emergencyWithdraw(address token, uint256 value) external onlyDao {
        IBEP20(token).transfer(address(dao()), value);
    }
    ...
    function balanceCheck() private {
        Require.that(
            Pancakev2().balanceOf(address(this)) >= totalStaged().add(totalBonded()),
            FILE,
            "Inconsistent PANCAKE-V2 balances"
        );
    }
...
contract Getters is State {
    ...
    function name() public view returns (string memory) {
        return "Binance Agile Set Dollar Stake";
    }
