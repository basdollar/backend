...
contract Pool is PoolSetters, Liquidity {
    ...
    constructor(address dollar, address cakelp) public {
        _state.provider.dao = IDAO(msg.sender);
        _state.provider.dollar = IDollar(dollar);
        _state.provider.cakelp = IERC20(cakelp);
    }
    ...
    event Provide(address indexed account, uint256 value, uint256 lessBusd, uint256 newCakelp);

    function deposit(uint256 value) external onlyFrozen(msg.sender) notPaused {
        cakelp().transferFrom(msg.sender, address(this), value);
    ...
    function withdraw(uint256 value) external onlyFrozen(msg.sender) {
        cakelp().transfer(msg.sender, value);
    ...
    function provide(uint256 value) external onlyFrozen(msg.sender) notPaused {
        ...
        (uint256 lessBusd, uint256 newCakelp) = addLiquidity(value);

        uint256 totalRewardedWithPhantom = totalRewarded().add(totalPhantom()).add(value);
        uint256 newPhantomFromBonded = totalRewardedWithPhantom.mul(newCakelp).div(totalBonded());

        incrementBalanceOfBonded(msg.sender, newCakelp);
        incrementBalanceOfPhantom(msg.sender, value.add(newPhantomFromBonded));


        balanceCheck();

        emit Provide(msg.sender, value, lessBusd, newCakelp);
    }
    ...
    function balanceCheck() private {
        Require.that(
            cakelp().balanceOf(address(this)) >= totalStaged().add(totalBonded()),
            FILE,
            "Inconsistent Cake-LP balances"
        );
    }
